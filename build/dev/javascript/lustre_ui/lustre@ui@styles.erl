-module(lustre@ui@styles).
-compile([no_auto_import, nowarn_unused_vars, nowarn_unused_function]).

-export([elements/0, theme/1]).

-spec elements() -> lustre@element:element(any()).
elements() ->
...
-spec scale_to_map(lustre@ui@colour:scale(), binary()) -> gleam@map:map_(binary(), binary()).
scale_to_map(Scale, Prefix) ->
    gleam@map:from_list(
        [{<<<<"$("/utf8, Prefix/binary>>/binary, ".app_background)"/utf8>>,
                gleam_community@colour:to_css_rgba_string(
                    erlang:element(2, Scale)
                )},
            {<<<<"$("/utf8, Prefix/binary>>/binary,
                    ".app_background_subtle)"/utf8>>,
                gleam_community@colour:to_css_rgba_string(
                    erlang:element(3, Scale)
                )},
            {<<<<"$("/utf8, Prefix/binary>>/binary, ".app_border)"/utf8>>,
                gleam_community@colour:to_css_rgba_string(
            ...
            {<<<<"$("/utf8, Prefix/binary>>/binary,
                ...
                gleam_community@colour:to_css_rgba_string(
                    erlang:element(5, Scale)
                )},
            {<<<<"$("/utf8, Prefix/binary>>/binary,
                    ".element_background_hover)"/utf8>>,
                gleam_community@colour:to_css_rgba_string(
                    erlang:element(6, Scale)
                )},
            {<<<<"$("/utf8, Prefix/binary>>/binary,
                    ".element_background_strong)"/utf8>>,
                gleam_community@colour:to_css_rgba_string(
                    erlang:element(7, Scale)
                )},
            {<<<<"$("/utf8, Prefix/binary>>/binary,
                    ".element_border_subtle)"/utf8>>,
                gleam_community@colour:to_css_rgba_string(
                    erlang:element(8, Scale)
                )},
            {<<<<"$("/utf8, Prefix/binary>>/binary,
                    ".element_border_strong)"/utf8>>,
                gleam_community@colour:to_css_rgba_string(
                    erlang:element(9, Scale)
                )},
            {<<<<"$("/utf8, Prefix/binary>>/binary, ".solid_background)"/utf8>>,
                gleam_community@colour:to_css_rgba_string(
                    erlang:element(10, Scale)
                )},
            {<<<<"$("/utf8, Prefix/binary>>/binary,
                    ".solid_background_hover)"/utf8>>,
                gleam_community@colour:to_css_rgba_string(
                    erlang:element(11, Scale)
                )},
            {<<<<"$("/utf8, Prefix/binary>>/binary,
                    ".text_high_contrast)"/utf8>>,
                gleam_community@colour:to_css_rgba_string(
                    erlang:element(12, Scale)
                )},
            {<<<<"$("/utf8, Prefix/binary>>/binary, ".text_low_contrast)"/utf8>>,
                gleam_community@colour:to_css_rgba_string(
                    erlang:element(13, Scale)
                )}]
    ).

-spec theme_to_map(lustre@ui:theme()) -> gleam@map:map_(binary(), binary()).
theme_to_map(Theme) ->
    _pipe = gleam@map:new(),
    _pipe@1 = gleam@map:merge(
        _pipe,
        scale_to_map(erlang:element(2, Theme), <<"primary"/utf8>>)
    ),
    _pipe@2 = gleam@map:merge(
        _pipe@1,
        scale_to_map(erlang:element(3, Theme), <<"greyscale"/utf8>>)
    ),
    _pipe@3 = gleam@map:merge(
        _pipe@2,
        scale_to_map(erlang:element(4, Theme), <<"error"/utf8>>)
    ),
    _pipe@4 = gleam@map:merge(
        _pipe@3,
        scale_to_map(erlang:element(6, Theme), <<"success"/utf8>>)
    ),
    _pipe@5 = gleam@map:merge(
        _pipe@4,
        scale_to_map(erlang:element(5, Theme), <<"warning"/utf8>>)
    ),
    gleam@map:merge(
        _pipe@5,
        scale_to_map(erlang:element(7, Theme), <<"info"/utf8>>)
    ).

-spec theme(lustre@ui:theme()) -> lustre@element:element(any()).
theme(Theme) ->
    _pipe = theme_to_map(Theme),
    _pipe@1 = gleam@map:fold(
        _pipe,
        <<"
:root {
...
"/utf8>>,
        fun gleam@string:replace/3
    ),
    lustre@element@html:style([], _pipe@1).
